...
router.post('/addToSchedulerEvents/:id', async (req, res) => {
  const { id } = req.params;
  const { name, selectedTime, timezone } = req.body; // Include timezone
  ...
  try {
    ...
    const newStart = toUTC(selectedTime, timezone);
    const newEnd = moment(newStart).add(1, 'hour').toDate(); // Assuming 1 hour duration

    // Check if the new event overlaps with any existing events
    const isOverlap = event.schedulerEvents.some(existingEvent => {
      const existingStart = toUTC(existingEvent.selectedTime, timezone);
      const existingEnd = moment(existingStart).add(1, 'hour').toDate(); // Assuming 1 hour duration
